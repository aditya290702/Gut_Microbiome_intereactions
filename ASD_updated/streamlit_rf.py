...
if "asd" not in abundance_df.columns:
    st.error("Missing 'ASD' column for classification.")
    st.stop()
...
st.write("### Top 10 Unique Genus-Level Features")
st.dataframe(top_unique_features)

# Plot top 10 unique features
st.write("### Top 10 Unique Features (Bar Chart)")
fig, ax = plt.subplots(figsize=(10, 6))
ax.barh(top_unique_features['Feature'], top_unique_features['SHAP Importance'], color='royalblue')
ax.set_xlabel("SHAP Importance")
ax.set_ylabel("Feature")
ax.set_title("Top 10 Unique Features")
ax.invert_yaxis()
st.pyplot(fig)
